...
app.get('/montre', (req, res) => {
  db.all(`
      ...
      if (err) {
          console.error("Erreur, les montres n'ont pas été trouvées :", err.message);
          res.status(500).json({ error: 'Internal server error' });
          return;
      }
...
app.post('/montre/add', (req, res) => {
  const {nom, boitier_texture, boitier_forme, bracelet_texture, pierre, main_color, createur } = req.body;

  // Vérifie si tous les paramètres sont renseignés
  if (!nom || !boitier_texture || !boitier_forme || !bracelet_texture || !pierre || !main_color || !createur) {
      res.status(400).json({ error: 'nom, boitier_texture, boitier_forme, bracelet_texture, pierre, main_color and createur are required' });
      return;
  }
  ...
  db.all(`
      ...
      if (montre.length == 0){
          db.run(`INSERT
              ...
              [nom, boitier_texture, boitier_forme, bracelet_texture, pierre, main_color, createur], function (err) {
              if (err) {
  ...
  db.all(`
      ...
      if(montre_existe.length > 0){
          db.all(`
              ...
              WHERE userID = ?
              AND montreID = ?
              `, [id_user, id_montre], (err, panier) => {
  
              if (err) {
                  console.error("Erreur, le panier n'a pas pue être récupéré : ", err.message);
                  res.status(500).json({ error: 'Internal server error' });
                  return;
              }
              ...
              if (panier.length == 0){
                  db.run(`INSERT INTO Panier (userID, montreID) VALUES (?, ?)`, [id_user, id_montre], function (err) {
                  if (err) {
                      console.error('Error ajout panier:', err.message);
                      res.status(500).json({ error: 'Internal server error' });
                      return;
                  }
                  res.json({ message : "Montre ajoutée dans le panier avec succès."});
                  });
              } else {
...
app.put('/montre/:id_montre/modif', (req,res) => {
  ...
  // Vérifie si tous les paramètres sont renseignés
  if (!id_montre || !nom || !boitier_texture || !boitier_forme || !bracelet_texture || !pierre || !main_color || !createur) {
      res.status(400).json({ error: 'id_montre, nom, boitier_texture, boitier_forme, bracelet_texture, pierre, main_color and createur are required' });
      return;
  }
  ...
  db.all(`
      ...
      AND u.pseudo = ?
      `, [id_montre, createur], (err, montre_existe) => {

      if (err) {
          console.error("Erreur, la montre n'existe où ne vous impartient pas : ", err.message);
          res.status(500).json({ error: 'Internal server error' });
          return;
      }
      ...
      if(montre_existe.length > 0){
          db.run(`
              ...
              WHERE montreID = ?
              `, [nom, boitier_texture, boitier_forme, bracelet_texture, pierre, main_color, createur, id_montre], (err) => {
          ...
          res.json({ message: "Cette montre n'existe pas, ou n'a pas été créée par cet user." });
      }
  });
});
...
app.delete('/user/panier/supp/:id_user/:id_montre', (req, res) => {
  ...
  db.run(`DELETE FROM Panier WHERE userID = ? AND montreID = ?`, [id_user, id_montre], function (err) {
      if (err) {
          console.error("Erreur, la montre n'a pas pue être supprimé du panier : ", err.message);
          res.status(500).json({ error: 'Internal server error' });
          return;
      }
      
      if (this.changes === 0) {
          res.status(404).json({ error: "Utilisateur ou montre non trouvé. Rien n'a été supprimé." });
      } else {
...
app.delete('/user/panier/supp/:id_user', (req, res) => {
  ...
  db.run(`DELETE FROM Panier WHERE userID = ?`, [id_user], function (err) {
      ...
      if (this.changes === 0) {
          res.status(404).json({ error: "Utilisateur non trouvé. Rien n'a été supprimé." });
      } else {
...
app.delete('/montre/:id_montre/supp/:id_user', (req, res) => {
  ...
  db.all(`SELECT * FROM Montre WHERE montreID = ? AND userID = ?`, [id_montre, id_user], function (err, montre) {
      ...
      if (montre.length > 0){
          db.all(`SELECT * FROM Panier WHERE montreID = ?`, [id_montre], function (err, montre_dans_panier) {
              ...
              if (montre_dans_panier.length > 0){
                  db.run(`DELETE FROM Panier WHERE montreID = ?`, [id_montre], function (err) {
                      if (err) {
                          console.error('Error delete panier:', err.message);
                          res.status(500).json({ error: 'Internal server error' });
                          return;
              ...
              // supprimer la montre
              db.run(`DELETE FROM Montre WHERE montreID = ?`, [id_montre], function (err) {
                  if (err) {
          ...
          res.json({ message: 'Une montre ne peut être supprimée que par son créateur.' });
      }
  })
});
